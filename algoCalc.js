...
(async () => {
    ...
    Utils.printOperationFee(asset_mint_fee, total_fee, `mint of ${assetsCount} ${assetsCount > 1 ? 'assets' : 'asset'}`, ALGORAND_CURRENT_PRICE);
    ...
    Utils.printOperationFee(account_min_balance_transfer, total_fee, `transfering minimum balance needed to receive the asset`, ALGORAND_CURRENT_PRICE);
    ...
    Utils.printOperationFee(receivers_opt_in, total_fee, `opting in ${receiversCount} ${receiversCount > 1 ? 'receivers' : 'receiver'} to the asset`, ALGORAND_CURRENT_PRICE);
    ...
    Utils.printOperationFee(transfer_asset, total_fee, `transfering ${assetsCount} ${assetsCount > 1 ? 'assets' : 'asset'} to ${receiversCount} ${receiversCount > 1 ? 'receivers' : 'receiver'}`, ALGORAND_CURRENT_PRICE);
    ...
    if (freeze === 'true') {
        ...
        Utils.printOperationFee(freeze_asset, total_fee, `freezing asset to ${receiversCount} ${receiversCount > 1 ? 'receivers' : 'receiver'}`, ALGORAND_CURRENT_PRICE);
    }
